...
struct mem_reserve {
	...
	int mem_reserve_size, mem_effective_size; /* # of pages */
...
extern int  (*rk_hypercall_hook)(unsigned long, unsigned long, unsigned long, unsigned long, unsigned long);
#ifdef CONFIG_RK_MEM
...
void rk_resource_set_start_account(rk_resource_set_t rs, struct task_struct*);
void rk_resource_set_stop_account(rk_resource_set_t rs, cpu_tick_t now);
...
void cpu_reserve_start_account(rk_reserve_t, struct task_struct*);
