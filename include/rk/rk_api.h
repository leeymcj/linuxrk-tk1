...
#if defined(__i386__) || defined(__i386) || defined(i386) || defined(__I386__) || defined(ARCH_i386)
	#include <arch/x86/include/generated/uapi/asm/unistd_32.h>
#elif defined(__amd64__) || defined(__amd64) || defined(amd64) || defined(__AMD64__) || defined(ARCH_x86_64)
...
static inline int rk_resource_set_create(char *name, int inherit_flag, int cleanup_flag, int cpursv_policy) {
...
static inline int rk_mem_reserve_create(int rd, mem_reserve_attr_t usr_mem_attr) {
	return syscall(__NR_rk_mem_reserve_create, rd, usr_mem_attr);
}
...
/* RK APIs: VChannel */
static inline int rk_vchannel(int type, int nr, void *data) {
	return syscall(__NR_rk_vchannel, type, nr, data);
}
static inline int rk_vchannel_register_host(int rd, int cpursv_idx, char *path) {
	long msg[2];
	msg[0] = cpursv_idx,
	msg[1] = (long)path;
	return rk_vchannel(RK_VCHANNEL_SYSCALL_REGISTER_HOST, rd, msg);
}
static inline int rk_vchannel_register_guest(int cpunum, char *path) {
...
static inline void rt_suspend_until_rsv_start() {
