...
asmlinkage  int sys_rk_resource_set_attach_process(int rd, pid_t pid, struct rk_ordered_list *req_cpursv_list)
{
	...
	if (cpursv_list->n == 0) {
		// No migration
		if (rset->cpursv_policy == CPURSV_NO_MIGRATION) {
			...
			if (rset->nr_cpu_reserves <= 0) {
				...
				cpunum = ((cpu_reserve_t)rset->cpu_reserves[min_count_id]->reserve)->cpu_res_attr.cpunum;
			}
		}
		// Default migration: tasks can use all cpursvs in rset
			...
			if (rset->nr_cpu_reserves > 0) {
				for (i = 0; i < RK_MAX_ORDERED_LIST; i++) {
					if (rset->cpu_reserves[i] == NULL) continue;
					cpunum = ((cpu_reserve_t)rset->cpu_reserves[i]->reserve)->cpu_res_attr.cpunum;
					break;
				}
			}
		}
		// Migration for fork-join model (CPURSV_MIGRATION_FORKJOIN)
			...
			if (rset->nr_cpu_reserves <= 0) {
				...
				if (list_empty(&rset->task_list)) {
					...
					cpunum = ((cpu_reserve_t)rset->cpu_reserves[master_string_id]->reserve)->cpu_res_attr.cpunum;
				}
				// parallel task
				else {
					if (min_count_id >= 0) {
		...
		if (rset->nr_cpu_reserves > 0) {
			cpunum = ((cpu_reserve_t)rset->cpu_reserves[(int)cpursv_list->elem[0]]->reserve)->cpu_res_attr.cpunum;
		}
	}
	...
	// Note: Set CPU core ID is not needed, because it will be set when the CPU reserve starts
	/*
	// Set the task default cpu core
	if (cpunum >= 0) {
		cpumask_t cpumask;
		cpus_clear(cpumask);
		cpu_set(cpunum, cpumask);
		//printk("rk_resource_set_attach_process: cpunum %d\n", cpunum);
		set_cpus_allowed_ptr(task, &cpumask);
	}*/
#endif	
	//printk("[] attached: pid %d, curpid %d, curcpu %d, taskcpu %d\n", task->pid, current->pid, smp_processor_id(), cpunum);
...
asmlinkage  int sys_rk_resource_set_detach_process(int rd, pid_t pid)
{
	...
	if (rset->nr_cpu_reserves > 0) restore_prio = TRUE;
	...
	if (restore_prio) {
		if (task->rk_mutex_nested_level <= 0) {
			...
			sched_setscheduler_nocheck(task, SCHED_NORMAL, &par);
		}
		else {
			task_original_prio(task) = 0;
		}
	}
