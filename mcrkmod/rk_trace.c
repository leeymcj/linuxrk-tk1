...
static inline void rk_trace_set_sched_data(struct rk_trace_data_sum *p, bool flag)
{
	...
	if (p->nr_sched < RK_TRACE_SUM_NR_HISTORY) {
		...
		p->sched_core[index] = smp_processor_id();
...
int sys_rk_mem_reserve_show_task_vminfo(int pid)
{
	...
	//int mapcount = 0;
	int n_mem_reserve = 0;
	int n_mem_file = 0, n_mem_cache = 0, n_mem_normal = 0;
	int n_estimated_total = 0;
	int n_shared = 0;
	//unsigned long vm_flags;
	int color_usage[RK_MEM_MAX_COLOR] = {0,};
	int bank_color_usage[RK_MEM_MAX_COLOR] = {0,};
	...
	*/
	...
	while(mmap) {
		...
		printk("##### Mem Region: mmap->vm_start:%lx - %lx (%lu bytes) - F:%lx", mmap->vm_start, mmap->vm_end, mmap->vm_end - mmap->vm_start, mmap->vm_flags);
		...
		while(n < mmap->vm_end)	{
			...
			if (!pte_present(*pte)) {
				#if 0
				unsigned long flag_mask = (1 << 12) - 1;
				printk("%lx|evicted page(pte_flag:%lx)\n", n, pte_flags(*pte) & flag_mask);
				#endif
			}			
			else {
				struct page *page;
				...
				unsigned long phy_addr2;
				page = pte_page(*pte);
				//phy_addr = pte_val(*pte) & PTE_PFN_MASK;
				phy_addr2 = page_to_pfn(page);
				...
				if (PageMemReserve(page)) {
					...
					nr_referenced = 0;
					#if 0
					printk("%lx|memrsv page(%lx)%lx pfn:%lx, f:%x, c:%d, mc:%d, rsv:%d, %s, shr:%d, ac:%d, ref:%d\n", 
						n, pte_flags(*pte) & flag_mask, (unsigned long)page, phy_addr2, 
						(unsigned int)page->flags, page_count(page), 
						page_mapcount(page), PageMemReserve(page), 
						page->mapping == NULL ? "cache" : 
						(((unsigned long)page->mapping & 0x1) ? "mem" : "file"),
				...
				if (page->mapping == NULL) n_mem_cache++;
				else if ((unsigned long)page->mapping & 0x1) n_mem_normal++;
				else n_mem_file++;
			}
...
next_vma:
	...
	printk("       - n_normal_mem(anon): %d, mem-cache: %d, file-mapped(page-cache): %d\n", n_mem_normal, n_mem_cache, n_mem_file);
...
int sys_rk_mem_reserve_show_reserved_pages(rk_resource_set_t rset)
{
	struct mem_reserve_page *entry;
	int i;
	mem_reserve_t mem = rset->mem_reserve->reserve;
	int measured_free = 0;
	int measured_act = 0;
	...
	printk("[Memory Reservation Info]\n");
	printk("total(eff/tot): %d/%d, free:%d, used:%d\n", 
		mem->mem_effective_size, mem->mem_reserve_size, mem->mem_free_size, mem->mem_used_size);
	...
	printk("summary - total(eff/tot): %d/%d, free:%d, used:%d (act:%d, inact:%d), conserved:%d\n", 
		mem->mem_effective_size, mem->mem_reserve_size, mem->mem_free_size, 
...
void __rk_pmc_init(void) 
{
	...
	for (i = 0; i < num_cpus; i++) {
		...
		{                        
			...
			//printk("L2X0_CNT0: %x, CNT1: %x\n", l2_hit, l2_ref);

			//val = readl(IO_ADDRESS(L2_BASE_ADDR + L2X0_AUX_CTRL));
			//printk("L2X0_CTRL: %x\n", val);
			...
			//val = readl(IO_ADDRESS(L2_BASE_ADDR + L2X0_EVENT_CNT1_CFG));
			//printk("L2X0_EVENT_CNT1_CFG: %x\n", val);
			//val = readl(IO_ADDRESS(L2_BASE_ADDR + L2X0_EVENT_CNT0_CFG));
			//printk("L2X0_EVENT_CNT0_CFG: %x\n", val);
			//val = readl(IO_ADDRESS(L2_BASE_ADDR + L2X0_EVENT_CNT_CTRL));
			//printk("L2X0_EVENT_CNT_CTRL: %x\n", val);
		}
#else
		// Instructions
		pmc_attr.config = PERF_COUNT_HW_INSTRUCTIONS;
...
#ifdef CONFIG_RK_MEM
	case 0x200:
		ret = sys_rk_mem_reserve_show_reserved_pages(resource_set_descriptor[nr]);
		break;
	case 0x201:
