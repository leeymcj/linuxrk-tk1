...
#define	CAPACITY_OF(c,t)	(div_s64(INT2CAPACITY(c),(t)))
	...
	list_for_each_entry(rsv_i, ptr_cpu_reserves_head(cpunum), cpu_link) {
		...
		while (timespec_le(prev_completion_time, rsv_i_attr->deadline) && timespec_lt(prev_completion_time, completion_time)) {
			...
			list_for_each_entry(rsv_j, ptr_cpu_reserves_head(cpunum), cpu_link) {
				...
				result = timespec_mult(rsv_j_attr->compute_time, ceil_value);
...
void cpu_reserve_replenish(rk_reserve_t rsv, cpu_tick_t start_ticks, cpu_tick_t output_period) 
{
	...
	if (rsv->reservation_state == RSV_IS_NULL) {
		...
		if (rsv->reservation_state & RSV_IS_RUNNING) {
			...
			/* The task is not running, so get the previous information */
		...
		utilization = (unsigned long)div_u64(cpu->used_ticks_in_cur_period * 10000, cpu->cpu_period_ticks);
	...
	cpursv_profile_update(&now, NULL, NULL, &cpu->cpu_profile);
	...
	*output_period = cpu->cpu_period_ticks;
	...
	list_for_each_entry(task, &rset->task_list, rk_resource_set_link) {
		...
		if (nr_task_count == 0 && !(rsv->reservation_state & RSV_IS_RUNNING)) {
			if (debug_rd == rset->rd_entry) {
				printk("replenish: curcpu %d, pid %d tsk_state %ld rk_state %d tsk_cpu %d (act)\n", smp_processor_id(), task->pid, task->state, task->rk_cannot_schedule, task_cpu(task));
			}
			task->rk_cannot_schedule &= ~RK_TASK_SCHED_MASK;
		...
		rk_push_to_workqueue(cpunum, type, task, (void*)(long)cpunum, (void*)(long)cpu->cpu_priority_index);
	}
...
void cpu_reserve_enforce(rk_reserve_t rsv)
{
	...
	if (debug_rd == rset->rd_entry) {
		printk("enforce: curcpu %d, rsvidx %d\n", smp_processor_id(), rsv->reserve_index);
	}
	list_for_each_entry(task, &rset->task_list, rk_resource_set_link) {
		rk_reserve_t newrsv;
		...
		for (;;) {
			...
			if (newrsv == rsv) break;
			if ((newrsv->reservation_state & RSV_IS_STARTED) && !(newrsv->reservation_state & RSV_IS_DEPLETED)) break;
		}
		...
		if (newrsv != rsv) {
			...
			if (!(newrsv->reservation_state & RSV_IS_RUNNING) && !newcpu->waking_task) {
				if (debug_rd == rset->rd_entry) {
					printk("enf: curcpu %d, pid %d tsk_state %ld rk_state %d tsk_cpu %d newrsv %d (act)\n", smp_processor_id(), task->pid, task->state, task->rk_cannot_schedule, task_cpu(task), newrsv->reserve_index);
				}
				type |= RK_WORK_WAKEUP;
				task->rk_cannot_schedule &= ~RK_TASK_SCHED_MASK;
				cpunum = task_cpu(task); 
 ...
 * - rk_sched.c::rk_schedule 
 */ 
void cpu_reserve_start_account(rk_reserve_t rsv, struct task_struct *task)
{
	cpu_reserve_t cpu;
	cpu_tick_data_t next;
	int cpunum;
	...
	cpu = rsv->reserve;
	cpunum = cpu->cpu_res_attr.cpunum;
	//printk("start: pid %d prio %d cpunum %d rsvid %d\n", task->pid, task->rt_priority, smp_processor_id(), rsv->reserve_index);
...
asmlinkage int sys_rk_cpu_reserve_create(int rd, cpu_reserve_attr_t cpu_attr)
{
	...
	if (current->rk_resource_set && current->rk_resource_set->nr_cpu_reserves) {
...
static inline void cpursv_profile_utilization_helper(struct rk_cpu_profile_set *profile, cpu_reserve_t cpu)
{
	...
	if (profile->used_ticks > 0) {
		profile->cur_buf[profile->cur_pos].utilization 
			= div_s64(profile->used_ticks * 10000, cpu->cpu_period_ticks);
	}
	else {
...
static inline int rk_getcpursv_start_profile_helper(struct rk_cpu_profile_set *profile, int size)
{
	...
	profile->buf[0] = kmalloc(size * sizeof(struct rk_cpu_profile), GFP_ATOMIC);
	if (profile->buf[0] == NULL) {
		return RK_ERROR;
	}
	profile->buf[1] = kmalloc(size * sizeof(struct rk_cpu_profile), GFP_ATOMIC);
	if (profile->buf[1] == NULL) {
		kfree(profile->buf[0]);
		return RK_ERROR;
	}
	memset(profile->buf[0], 0, size * sizeof(struct rk_cpu_profile));
	memset(profile->buf[1], 0, size * sizeof(struct rk_cpu_profile));
	profile->cur_buf = profile->buf[0];
