...
void rk_mutex_change_task_priority(struct task_struct *task, int need_indirect)
{
	...
	if (need_indirect == FALSE) {
		...
		if (par.sched_priority) policy = cpu_reserves_kernel_scheduling_policy;
		else policy = SCHED_NORMAL;
...
// Called by
// - rk_mutex_open
// - rk_hypercall_handler (NR_rk_create_vcpu_inherited_prio_list)
...
int rk_intervm_mutex_open(int type, int key, int mode) 
{
	...
	if (kvm_hypercall3(NR_rk_intervm_mutex_open, type, key, mode) == RK_ERROR) {
...
int rk_intervm_mutex_destroy(int type, int key)
{
	...
	// Request to the host machine
	if (kvm_hypercall2(NR_rk_intervm_mutex_destroy, type, key) == RK_ERROR) {
...
retry_locking:
	...
	// Request to the host machine
	if (is_trylock) 
		ret = kvm_hypercall4(NR_rk_intervm_mutex_trylock, type, mid, current->pid, task_inherited_prio(current));
	else
		ret = kvm_hypercall4(NR_rk_intervm_mutex_lock, type, mid, current->pid, task_inherited_prio(current));
...
{ 
	...
	ret = kvm_hypercall3(NR_rk_intervm_mutex_unlock, type, mid, current->pid);
	if (ret == RK_ERROR) {
...
int rk_intervm_mutex_unlock_all(struct task_struct *task) 
{
	...
	return kvm_hypercall1(NR_rk_intervm_mutex_unlock_all, task->pid);
}
...
int rk_intervm_mutex_remove_from_waitlist(int mid, struct task_struct *task) 
{
	...
	// unlock all mutexes locked by that task
	return kvm_hypercall2(NR_rk_intervm_mutex_remove_from_waitlist, mid, task->pid);
}
