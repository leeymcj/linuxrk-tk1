	...
	if (((volatile rk_resource_set_t*)prev->rk_resource_set)) {	
		...
		if (prev->rk_resource_set->rd_entry == debug_rd) {
			printk("prev: pid %d state %ld rk_state %d currsv %d taskrsv %d tskrsvstt %d cpuid %d\n", 
				...
				cpursv ? cpursv->reserve_index : -1, cpursv ? cpursv->reservation_state : -1, smp_processor_id());
		}
	}
	...
	if (((volatile rk_resource_set_t*)next->rk_resource_set)) {	
		...
		if (cpursv) {
			...
			// Check if rsv is still being used by another task. 
			         ...
			         || ((cpu_reserve_t)cpursv->reserve)->cpu_res_attr.cpunum != smp_processor_id()) {
				// Reschedule task
				set_tsk_need_resched(next);
			}
#else
			...
			// Check if rsv is still being used by another task. 
				...
				if (next->rk_resource_set->rd_entry == debug_rd) {
					printk("next: pid %d state %ld rk_state %d rsvstt %d cpuid %d (suspend/cur %d wait %d)\n", 
						next->pid, next->state, next->rk_cannot_schedule, cpursv->reservation_state, smp_processor_id(), 
				...
				cpu_reserve_start_account(cpursv, next);
			}
		}
	}
...
void rk_fork(struct task_struct *tsk)
{
	...
	if (tsk->group_leader->rk_resource_set) {
		rd = tsk->group_leader->rk_resource_set->rd_entry;
		inherit_flag = tsk->group_leader->rk_resource_set->rk_inherit;
		if (inherit_flag) printk("rk_fork : pid %d - parent(tgid): %d, inherit: %d\n", tsk->pid, tsk->group_leader->pid, inherit_flag);
	}
	if (inherit_flag == FALSE && tsk->real_parent->rk_resource_set) {
		rd = tsk->real_parent->rk_resource_set->rd_entry;
		inherit_flag = tsk->real_parent->rk_resource_set->rk_inherit;
		if (inherit_flag) printk("rk_fork : pid %d - parent: %d, inherit: %d\n", tsk->pid, tsk->real_parent->pid, inherit_flag);
	}
	if (rd < 0) return; 
	...
	if (inherit_flag) {
		...
		// Give normal priority
		struct sched_param par;
		par.sched_priority = 0;
		sched_setscheduler_nocheck(tsk, SCHED_NORMAL, &par);
